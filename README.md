...
- Pre-worker synchronization transactions may resume when their result can be proven from persisted state and Git observations. A safely normalized `service_shutdown` or `process_loss` execution may automatically resume in its existing workspace with a fresh worker; operator aborts and ordinary failures still require explicit retry. Ambiguous worker ownership and later execution stages remain fail-closed.
